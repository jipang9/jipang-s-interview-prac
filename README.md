...
16. 트리, 그래프를 비교하여 설명해주실 수 있을까요?
    ...
    트리와 그래프는 간선과 노드를 연결해서 구성하는 자료구조입니다. 이 구조를 이용해서 노드 간 관계를 표현할 수 있는 자료구조입니다. 이 관계 속에서 차이점이 있는데,
    트리 같은 경우엔 노드 사이에 1개만의 경로를 갖지만, 그래프는 2개 이상의 경로를 가질 수 있습니다. 또한 트리는 하나의 부모 노드로 이루어져 있지만 그래프는 부모-자식 
    개념이 없습니다. 그래프는 루트 개념이 없습니다, 그러나 트리는 하나의 루트가 존재합니다. 그래프는 순환, 비순환의 구조를 이루는데, 트리는 방향성이 있으며 사이클이 
    존재하지 않습니다. 그래서 비순환 그래프입니다.
    
17. 모든 요소에 인덱스를 걸지 않는 이유는 무엇일까요? 
    
    모든 요소에 인덱스를 적용하면 저장 공간을 많이 사용할 것입니다. 이유는 인덱스를 관리하기 위해서 데이터베이스가 추가로 저장 공간을 할당하는데, 불필요한 요소까지 인덱스가
    걸려있음으로써 자원의 낭비가 심해질 것입니다. 또한 수정, 삭제, 삽입이 빈번하게 발생하는 테이블에 인덱스를 걸게되면 결국 새로운 데이터에 관해서 인덱스의 업데이트가 필요
    할 텐데, 여기에서도 추가적인 리소스가 많이 소모될 것입니다. 또한 이전 인덱스를 제거하지않고 사용하지 않음으로 처리해버리니까 낭비가 발생할 것입니다.

18. 트랜잭션이란 무엇이고 원자성, 일관성, 고립성, 지속성이란 무엇인지 설명해주실 수 있을까요?

    트랜잭션을 작업을 처리하는 하나의 단위입니다. db의 입장에서는 하나의 커넥션을 하나의 세션으로 처리를 하게 되는데, 작업을 하나로 묶지 않으면 각기 다른 커넥션으로 생각할
    것입니다. 그래서 이를 묶어 하나의 작업 단위로 만드는 것이 트랜잭션 입니다. 이 트랜잭션에는 특징이 있습니다. 바로 원자성, 일관성, 고립성, 지속성입니다. 원자성은 한 트랜
    잭션 내 실행한 작업들은 하나로 간주해서 모두 성공하거나 실패하는 것으로 처리합니다. 데이터베이스는 항상 일관성 있는 상태를 유지해야하는데, 이 것이 일관성 입니다. 고립성
    은 실행되는 트랜잭션끼리 서로 영향을 미치지 않도록 격리하는 것입니다. 지속성, 영속성은 결과를 항상 영구적으로 저장해야 함을 의미합니다.
